...
Functions, 2026-08-12): this page used to be a single "Add plant" expander
followed by the browse/edit/delete table, with no CSV/Excel import at all.
...
PLANT_REQUIRED_COLUMNS = ["name"]
PLANT_OPTIONAL_COLUMNS = ["plant_code", "location", "notes"]
...
def _target_company(key):
    """Company a new/imported plant should be created under. Locked to the
    user's own company for non-platform-owners; for the platform owner,
    uses the current company filter if one is picked, otherwise asks which
    company this record belongs to (required when viewing 'All
    companies')."""
...
tab_create, tab_edit_delete, tab_import = st.tabs(cr11_function_tab_labels("Plant"))

with tab_create:
    if not page_usable:
        st.caption("View-only access - adding a plant is restricted for your role.")
    else:
        plant_company = _target_company("add_plant_company")
        if limit_reached:
            st.warning(
...
with tab_import:
    if not page_usable:
        st.caption("View-only access - importing plants is restricted for your role.")
    else:
        import_target_company = _target_company("import_plant_company")
        show_pending_banner("plant_import_msg")
        pdf, pfilename = csv_excel_uploader(PLANT_REQUIRED_COLUMNS, PLANT_OPTIONAL_COLUMNS, key="plant_upload")
        if pdf is not None and not import_target_company:
            st.error("Pick a company above before importing.")
        elif pdf is not None:
            import_limit_reached = (
                max_plants is not None
                and import_target_company is company_filter
                and plant_count_for_company is not None
                and plant_count_for_company >= max_plants
            )
            if import_limit_reached:
                st.warning(
                    f"{import_target_company.name} is already at its subscription's plant limit "
                    f"({max_plants} plants). Upgrade the subscription or remove an unused plant "
                    "before importing more."
                )
            else:
                existing_names = {
                    p.name.strip().lower()
                    for p in session.query(Plant).filter(Plant.company_id == import_target_company.id).all()
                }
                good_rows, bad_rows = [], []
                ...
                if bad_rows:
                    st.warning("Flagged rows have no plant name.")
                    render_data_table(pd.DataFrame(bad_rows), max_height="300px")

                if good_rows and st.button("Confirm import", key="confirm_plant_import"):
                    new_rows, dup_rows = dedupe_import_rows(
                        good_rows, existing_names, key_func=lambda row: str(row["name"]).strip().lower()
                    )
                    for row in new_rows:
                        session.add(
                            Plant(
                    ...
                    session.commit()
                    clear_scope_cache()
                    msg = f"Imported {len(new_rows)} plant(s) from {pfilename}."
                    if dup_rows:
                        msg += f" Skipped {len(dup_rows)} row(s) already recorded for this company (likely a repeat click)."
                    set_pending_banner("plant_import_msg", msg)
                    st.rerun()
