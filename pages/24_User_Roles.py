...
Functions, 2026-08-12): this page used to be a single "Add custom role"
expander followed by the roles table/edit panel, with no CSV/Excel import
at all. Restructured into the mandated 3 tabs (Create/Edit-Delete/Import)
via cr11_function_tab_labels("Role"). The net-new CSV/Excel import creates
custom roles only (built-in roles are never created this way, same as the
manual Create tab) with the same default page access every manually-added
role gets (Full access on every page until an admin narrows it on the
Edit/Delete tab) - importing bulk roles is a naming/description shortcut,
not a shortcut around reviewing per-page access."""
...
ROLE_REQUIRED_COLUMNS = ["name"]
ROLE_OPTIONAL_COLUMNS = ["description", "company_id"]
...
render_function_action_intro(
    ...
    action_text=(
        ...
        "use Production Run and Quality screens), or use CSV/Excel import to bulk-create several "
        "custom roles by name/description at once - every imported role starts with Full access on "
        "every page, same as a manually-added one, so review and narrow access afterward. Click a "
        "role below to edit its per-page access, or its name/description for custom roles - "
        "built-in roles can only have their page access adjusted, not their name."
    ),
)
session = get_session()
user = current_user()
...
tab_create, tab_edit_delete, tab_import = st.tabs(cr11_function_tab_labels("Role"))
...
with tab_import:
    show_pending_banner("role_import_msg")
    valid_company_ids = {c.id for c in companies} if is_platform_owner else {own_company_id}
    rdf, rfilename = csv_excel_uploader(ROLE_REQUIRED_COLUMNS, ROLE_OPTIONAL_COLUMNS, key="role_upload")
    if rdf is not None:
        good_rows, bad_rows = [], []
        for _, row in rdf.iterrows():
            name_val = str(row.get("name", "") or "").strip()
            company_val = row.get("company_id")
            if is_platform_owner:
                target_id = int(company_val) if pd.notna(company_val) else None
                row_ok = bool(name_val) and target_id in valid_company_ids
            else:
                # Non-platform-owner admins can only create roles for their
                # own company - any company_id column value is ignored (same
                # as the manual Create tab, which never lets them pick one).
                target_id = own_company_id
                row_ok = bool(name_val)
            if row_ok:
        ...
        if bad_rows:
            st.warning(
                "Flagged rows have no role name, or (platform owner only) reference an unknown "
                "company_id."
            )
            render_data_table(pd.DataFrame(bad_rows), max_height="300px")

        if good_rows and st.button("Confirm import", key="confirm_role_import"):
            existing_keys = {
                (r.company_id, r.name.strip().lower())
                for r in session.query(Role).filter(
                    Role.company_id.in_(valid_company_ids) if is_platform_owner else Role.company_id == own_company_id
                ).all()
            }
            ...
            new_rows, dup_rows = dedupe_import_rows(
                ...
                key_func=lambda row: (_row_company_id(row), str(row["name"]).strip().lower()),
            )
            for row in new_rows:
                target_company_id = _row_company_id(row)
                new_role = Role(
                    company_id=target_company_id,
                ...
                audit_log.log_role_change(
                    session, target_type="role",
                    change_summary=f"Created custom role '{new_role.name}' (CSV/Excel import)",
                    changed_by_user_id=user["id"], company_id=target_company_id,
                    target_id=new_role.id, target_label=new_role.name,
                )
            session.commit()
            msg = f"Imported {len(new_rows)} role(s) from {rfilename}."
            if dup_rows:
                msg += f" Skipped {len(dup_rows)} row(s) already recorded for their company (likely a repeat click)."
            set_pending_banner("role_import_msg", msg)
            st.rerun()
